...
const upgrades = [
...

    {
        ...
        purchased: 0,
        maxpurchases: 1,
        onpurchase: () => { document.getElementById("energyContainer").classList.remove("hidden") },
    ...
    {
        name: "Unlock Mineshaft 1: Basic tunnel",
        id: "unlockshaft1",
        cost: [["matter", 50]],
        unlocked: false,
        reqs: [["barcoins", 1]],
        flavortext: "Dig out the first mineshaft out of many more.",
        purchased: 0,
        maxpurchases: 1,
        onpurchase: () => {},
...
for (const btns of upgrades) {
    ...
    button.id = btns.id
    if (btns.type === "basic") {
        button.style.backgroundColor = `#6a5409`
        document.getElementById("upgradeContainer").appendChild(button);
    } else if (btns.type === "mining") {
        button.style.backgroundColor = `#24387e`
        document.getElementById("miningUpgradeContainer").appendChild(button);
    }
